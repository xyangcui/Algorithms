...
def DA_test(DA):
    nm = 0; nc = 100
    xa_start = DA.four_dims_var_optimizer_scipy(initial_state[:,nm,nc],B_perturb,observation[:,:,nm,nc],R_perturb,obs_idx,n,Dh,h,max_iter=150,tol=1e-6)
    xa = np.zeros((K,nt+1))
    xa[:,0] = xa_start.copy()
    for i in range(nt):
    ...
    x_truth = real[:,forecast_idx[nc]:forecast_idx[nc]+nt+1]
